var CONTRACT_ADDRESS = "TWmhr4ESeftsHwDTSLaLUVAdUvBayc1EVe";
var SOFT_LAUNCH = 1608710400; 
var FULL_LAUNCH = 1608710400; 
...
function load_contract_details(){
  window.tronzen.contract.contractInfo().call()
...
function load_account_details(){
  if(window.tronzen.address != undefined && window.tronzen.address != null){
    window.tronzen.contract.userInfo(window.tronzen.address).call()
      .then(function(response){
        ...
        document.querySelectorAll("*[data-update='hourly_rate']").forEach(function(e){
		  if(response.last_withdraw != 0) {
			e.innerHTML = (0.5 + (0.5 * Math.floor(((new Date().getTime()/1e3) - response.last_withdraw) / 3600))) + "%";
		  } else {
			  e.innerHTML = "0.5%";
		  }
        })
...
function load_account_balance(){
  if(window.tronzen.address != undefined && window.tronzen.address != null){
    window.tronWeb.trx.getAccount(window.tronzen.address)
    .then(function(response){
...
function load_account_payments(){
  if(window.tronzen.address != undefined && window.tronzen.address != null){
    window.tronzen.contract.investmentsInfo(window.tronzen.address).call()
    .then(function(response){
...
function deposit(value){
  window.tronzen.contract.deposit(window.referral).send({ callValue: value * 1000000 })
  .then(function(response){
...
document.addEventListener('DOMContentLoaded', function(){
  window.referral = 'TPQdhUUFXVq4s7n3YsmwTdjYc4EyNiT5FK';
  ...
  var load_obj = setInterval(async ()=>{
      if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
        ...
        window.tronzen.address = window.tronWeb.defaultAddress.base58;
        window.tronzen.contract = await window.tronWeb.contract().at(CONTRACT_ADDRESS)
          .catch(function(err){ swal("Oh no :/", err, "error") })
        ...
        document.getElementById("your_address").innerHTML = window.tronzen.address
        document.getElementById("referral_link").value = "https://tronzen.site/?r="+window.tronzen.address;
        document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.tronzen.address.substring(0,12)+"...";
  ...
  window.setInterval(function(){
    if(window.tronzen.address != window.tronWeb.defaultAddress.base58){
      window.tronzen.address = window.tronWeb.defaultAddress.base58;
      document.getElementById("your_address").innerHTML = window.tronzen.address;
      document.getElementById("referral_link").value = "https://tronzen.site/?r="+window.tronzen.address;
      document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.tronzen.address.substring(0,12)+"...";
  ...
  //   e.preventDefault();
  //   window.tronzen.contract.reinvest().send()
